...
import { Heart, ArrowDown, MapPin, Clock, CalendarHeart, Sparkles } from "lucide-react";
...
/* ─── Floating Lotus Petals (Enhanced) ─── */
const FloatingLotus = () => {
...
const PersonalNote = () => {
  ...
  return (
    <section className="min-h-[50vh] flex items-center justify-center relative z-30 px-6 py-12 md:py-20 w-full max-w-4xl mx-auto">
      ...
      >
        <CornerOrnaments />
        <p className="font-[family-name:var(--font-cinzel)] text-[#D4AF37] uppercase tracking-widest text-[10px] md:text-xs mb-6">
          A Personal Note
        </p>
        {payload.n && (
...
const FooterBlessings = ({ defaultFam }: { defaultFam: string }) => {
  ...
  return (
    <section className="min-h-[70vh] flex flex-col items-center justify-center relative z-30 p-8 text-center pb-24">
      ...
      <div className="w-full text-center mt-16 pointer-events-auto">
        <a
...
export default function DesignOne({ wedding }: DesignOneProps) {
  ...
  const arrowTop = useTransform(smoothProgress, scrollPoints, ["80%", "80%", "10%", "50%", "50%", "10%", "50%", "50%", "85%", "65%"]);
  const arrowScale = useTransform(smoothProgress, scrollPoints, [1, 1, 0.4, 1, 1, 0.4, 1, 1, 0.4, 1]);
  const leftDesktop = useTransform(smoothProgress, scrollPoints, ["50%", "50%", "50%", "25%", "25%", "25%", "75%", "75%", "75%", "50%"]);
  const leftMobile = useTransform(smoothProgress, scrollPoints, ["50%", "50%", "50%", "50%", "50%", "50%", "50%", "50%", "50%", "50%"]);
  const arrowLeft = isDesktop ? leftDesktop : leftMobile;

  const brideGlow = useTransform(smoothProgress, scrollPoints, [
  ...
  const groomGlow = useTransform(smoothProgress, scrollPoints, [
    "0px 0px 0px 0px rgba(212,175,55,0)",
    "0px 0px 0px 0px rgba(212,175,55,0)",
    "0px 0px 0px 0px rgba(212,175,55,0)",
    "0px 0px 0px 0px rgba(212,175,55,0)",
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ top: arrowTop, left: arrowLeft, scale: arrowScale, x: "-50%", y: "-50%" }}
  ...
  <div className="relative w-22 h-22 md:w-20 md:h-20">
    ...
    />
          ...
          {wedding.bride.traits && (
            <div className="space-y-2 text-center font-[family-name:var(--font-cormorant)] italic text-lg font-semibold text-[#8B1E41]/90">
              {wedding.bride.traits.map((trait, idx) => (
                <FadeIn key={idx} delay={0.4 + idx * 0.1} className="bg-[#FBF7F0] border border-[#D4AF37]/40 py-1 rounded-lg shadow-sm">
                  {trait}
                </FadeIn>
              ))}
            </div>
      ...
      <section className="min-h-[90vh] flex flex-col items-center justify-center relative z-30 p-4">
        ...
        <div className="absolute w-96 h-96 md:w-[600px] md:h-[600px] rounded-full border-2 border-[#D4AF37]/20 animate-spin-slow pointer-events-none" style={{ animationDuration: '60s' }} />
        ...
        <div className="text-center bg-white/90 backdrop-blur-md p-8 md:p-14 rounded-3xl shadow-2xl border-2 border-[#D4AF37]/40 max-w-2xl relative z-10">
          ...
          <WordFade
            delay={0.3}
            text={wedding.couple.quote || "Different hearts. Different worlds. One beautiful destiny."}
        ...
        <div className="text-center bg-white/90 backdrop-blur-md p-8 md:p-14 rounded-3xl shadow-2xl border-2 border-[#D4AF37]/40 max-w-2xl w-full relative">
          ...
          <div className="space-y-6 text-gray-800 font-[family-name:var(--font-cormorant)]">
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <p className="text-xl max-w-sm">{wedding.event.venueTitle}, {wedding.event.venueAddress}</p>
            </div>
          </div>
          <a
