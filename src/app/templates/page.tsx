...
function TemplatesShowcaseContent() {
  ...
  // Initialize with URL param if valid, fallback to first template
  const [activeTemplateId, setActiveTemplateId] = useState<string>(() => {
    if (templateQuery && templates.some((t) => t.id === templateQuery)) {
      return templateQuery;
    }
    return templates[0]?.id || "design-one";
  });
  ...
  // Sync state only when browser back/forward buttons are used
  useEffect(() => {
    if (templateQuery && templates.some((t) => t.id === templateQuery)) {
      setActiveTemplateId(templateQuery);
    }
  }, [templateQuery]);

  // Tab switch handler: updates preview state and updates URL cleanly
  const handleTemplateChange = (templateId: string) => {
    setActiveTemplateId(templateId);
    router.replace(`/templates?template=${templateId}`, { scroll: false });
  };

  const activeDefinition = getTemplate(activeTemplateId) || templates[0];
